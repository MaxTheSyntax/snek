...
fn main() {
    ...
    while !game_over {
        ...
        if head.0 < 0 || head.0 >= 20 || head.1 < 0 || head.1 >= 20 || snake.body.contains(&head) {
            game_over = true;
        }
...
fn draw_game(snake: &Snake, food: &(i32, i32), score: u32, direction: &Direction) {
    // Draw the game board
    for y in 0..20 {
        for x in 0..20 {
            let c = if snake.body.contains(&(x, y)) {
                'O' // Snake body
            } else if (x, y) == *food {
                '$' // Food
            } else {
